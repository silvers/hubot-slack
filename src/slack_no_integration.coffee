...
class SlackBot extends Adapter
  ...
  run: ->
    ...
    options =
      token: process.env.SLACK_WEB_API_TOKEN
      autoReconnect: true
      autoMark: false
      ignoreChannels: process.env.SLACK_IGNORE_CHANNELS || ''
  ...
  loggedIn: (self, team) =>
    ...
    @robot.name = process.env.SLACK_BOT_NAME
  ...
  message: (msg) =>
    ...
    # ignore channel
    return if (@options.ignoreChannels.split ',' || []).indexOf(channel.name) >= 0
